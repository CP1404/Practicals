...
from in [https://github.com/CP1404/KivyDemos]{.underline}.
...
[https://github.com/CP1404/KivyDemos]{.underline}
...
1.  The work that you do today should be saved, committed and pushed to
    your Practicals repository in the prac\_07 folder. The simplest
    thing is to just **copy everything from the demos zip you just
    downloaded into your prac\_07 folder**. Then only commit the work
...
2.  (Here\'s a very simple example)  
    Open and run the **hello\_world.py** file.  
    You should see a lovely black window with "HelloWorld" in the title.

3.  (Here\'s an advanced example)  
    Open and run the **popup\_demo.py** file (which uses
    **popup\_demo.kv**) and run that to see how it works. You don\'t
...
Open the **box\_layout\_demo.py** and **box\_layout.kv** files and run
...
4.  Now to add a button handler, you need to edit both the py and kv
    files. Look at the 'id\_demo' files for a good example of this...  
...
7.  Now in your handle\_greet function, change this label's text, like:

> **print**(**\"test\"**)
>
> self.root.ids.output\_label.text = **\"Hello \"**
...
> **... \"Hello \"** + self.root.ids.input\_name.text
>
...
Open **dynamic\_widgets.py/kv** to see how this can be done.
...
-   Create the widgets (e.g. buttons) in Python code, e.g. temp\_button
    = Button(text=name)

-   Add these new widgets using the **add\_widget** method, e.g.
    self.root.ids.entriesBox.add\_widget(temp\_button)

-   Bind a function callback when you make the widget object to add
    event handler code, e.g.
    temp\_button.bind(on\_release=self.press\_entry)
...
dynamic\_widgets example as it is too different. Your app won\'t have
...
Hint: BoxLayouts inside a BoxLayout, and a size\_hint\_x...
